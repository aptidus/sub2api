...
//
// 条目顺序：密钥 → 文档 → 用量 → 支付/订单 → 资料。
// 常规用户只保留上线所需的客户入口，避免暴露管理员运营界面的复杂度。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/api-docs', label: t('nav.apiDocs'), icon: OrderListIcon },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/api-docs', label: t('nav.apiDocs'), icon: OrderListIcon },
