<template>
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      ...
      <button @click="router.push('/api-docs')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-cyan-100 transition-transform group-hover:scale-105 dark:bg-cyan-900/30">
          <Icon name="book" size="lg" class="text-cyan-600 dark:text-cyan-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('dashboard.viewApiDocs') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('dashboard.useKeyWithDocs') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
