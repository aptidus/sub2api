...
const filteredGroups = computed(() => {
  ...
  if (props.platform) {
    const samePlatformGroups = result.filter((g) => g.platform === props.platform)
    if (samePlatformGroups.length === 0 && props.platform !== 'antigravity') {
      // Fallback: no same-platform groups available; show all so admins can still bind accounts.
      result = props.groups
    }
  }
  ...
  return includeSelectedGroups(result)
})
