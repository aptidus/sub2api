export default {
  ...
  dashboard: {
    ...
    generateNewKey: '复制或轮换已分配的 API 密钥',
    viewUsage: '查看使用记录',
    checkDetailedLogs: '查看详细的使用日志',
    viewApiDocs: '查看 API 文档',
    useKeyWithDocs: '使用您的密钥调用 Messages、Chat Completions 或 Responses',
    redeemCode: '兑换码',
  ...
  apiDocs: {
    eyebrow: '开发者文档',
    title: 'API 文档',
    description: '面向客户的安全 API 使用说明：使用已分配的密钥并调用 Sub2API。这里不会暴露管理员密钥、上游账号细节或内部成本数据。',
    createKey: '打开 API 密钥',
    step1Title: '1. 复制您的密钥',
    step1Body: '用户会获得已分配的 sk-... API 密钥。管理员密钥和上游账号信息必须只放在服务端。',
    step2Title: '2. 选择接口',
    step2Body: '根据客户端选择 Messages、Chat Completions 或 Responses。',
    step3Title: '3. 跟踪用量',
    step3Body: '系统会自动记录用量，用于计费和限额计算。'
  },
  ...
  keys: {
    title: 'API 密钥',
    description: '复制或轮换分配给您账户的 API 密钥',
    ...
    allStatus: '全部状态',
    createKey: 'API 密钥',
    provisionedTitle: '此密钥由账户预先分配',
    provisionedDescription: '普通用户不创建或配置密钥。您可以在此复制已分配密钥、查看用量，或在密钥泄露时轮换密钥。',
    rotateKey: '轮换密钥',
    rotateConfirmTitle: '确认轮换 API 密钥？',
    rotateConfirmMessage: "轮换 '{name}' 会立即停用旧密钥值并生成新密钥。用量历史、限额和计费记录保持不变。",
    keyRotatedSuccess: 'API 密钥已轮换并复制到剪贴板',
    ...
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
    createFirstKey: '您的账户需要先分配 API 密钥，才能调用 API。',
    noProvisionedKey: '此账户还没有分配 API 密钥。请联系管理员或支持团队为您分配。',
  ...
  admin: {
    ...
    dashboard: {
      ...
      actual: 'Stripe 实收',
      standard: '标准',
      accountCost: 'Stripe 上游成本',
      todayUsageProfit: '今日用量毛利',
      totalUsageProfit: '总用量毛利',
      chargedMinusCost: '{charged} Stripe 付费用量 - {cost} 上游成本',
      margin: '毛利率',
      internalUsageCost: '内部/测试用量成本',
      todayInternalUsageCost: '今日内部成本',
      apiDocsEyebrow: '上线文档',
      apiDocsTitle: '最新客户 API 文档',
      apiDocsDescription: '管理员专用入口：包含鉴权、模型列表、Messages、Chat Completions、Responses、Token 统计、用量查询和安全规则。',
    ...
    users: {
      ...
      updating: '更新中...',
      internalUsage: '内部',
      apiKeyInternalUsageHint: '此密钥的用量仍会记录为内部成本，但不会计入客户毛利。',
      columns: {
        user: '用户',
        id: 'ID',
        email: '邮箱',
      ...
      createApiKeyForUser: '为此用户创建密钥',
      createApiKeyForUserHint: '新密钥会属于这个用户，不会再挂到管理员账号下。',
      createApiKey: '创建密钥',
      apiKeyNamePlaceholder: '密钥名称',
      copyNewKeyNow: '请现在复制此密钥。完整密钥只会显示一次。',
      apiKeyCreated: '已为用户创建 API 密钥',
      ...
      markInternalUsage: '标记内部',
      markCustomerUsage: '标记客户',
      internalUsageChanged: '内部用量设置已更新',
      internalUsageChangeFailed: '内部用量设置更新失败',
      noUsersYet: '暂无用户',
      ...
      form: {
        ...
        internalUsage: '标记为内部用量',
        internalUsageHint: '用量仍会记录并计算成本，但不会计入客户毛利。用于管理员、本地测试、内部项目和项目密钥。'
      },
      adjustBalance: '调整余额',
  ...
  onboarding: {
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 Sub2API',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 Sub2API AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 复制已分配的 API 密钥</li><li>📋 查看 API 文档</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
