...
func TestGatewayOpenAICompatibleHandlersAllowBooleanStreamToContinue(t *testing.T) {
	...
	tests := []struct {
		...
		// 注：本地定制保留 /v1/responses 在 claude_code_only 分组下可用（SpearAgent/Codex
		// 走 Responses 协议访问 Claude Code 账号），因此不再断言 /v1/responses 会被 403。
		// 详见 gateway_handler_responses.go 注释。
		{
