...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if directModel, ok := service.ResolveAdminDirectModel(apiKey, reqModel); ok {
		...
		h.chatCompletionsErrorResponse(c, http.StatusForbidden, "permission_error", "Admin API key model calls only allow Opus-Latest or claude-opus-4-7")
		return
	}
	...
	for {
		...
		if !service.IsSyntheticAdminModelAPIKey(apiKey) {
