...
func (s *stubAdminService) AdminUpdateAPIKeyInternalUsage(ctx context.Context, keyID int64, internalUsage bool) (*service.APIKey, error) {
	for i := range s.apiKeys {
		if s.apiKeys[i].ID == keyID {
			s.apiKeys[i].InternalUsage = internalUsage
			k := s.apiKeys[i]
			return &k, nil
		}
	}
