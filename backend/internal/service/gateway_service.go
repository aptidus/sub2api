	...
	if parsed.MetadataUserID != "" {
		...
		if uid != nil && uid.SessionID != "" {
			slog.Debug("sticky.hash_source",
				...
				"session", shortSessionHash(uid.SessionID),
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	// Sticky scheduler diagnostics are debug-only to avoid noisy production logs.
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if account != nil && account.IsAnthropicAPIKeyPassthroughEnabled() {
		// APIKey passthrough: forward the raw body. The generic field filter
		// happens later in buildCountTokensRequestAnthropicAPIKeyPassthrough
		// via sanitizeCountTokensRequestBody, which intentionally keeps
		// max_tokens because Anthropic count_tokens accepts it.
		passthroughBody := parsed.Body.Bytes()
	...
	if shouldMimicClaudeCode {
		...
		// OAuth mimicry injects a synthetic max_tokens (~128000) for fingerprint
		// alignment in /v1/messages. Strip it on the count_tokens path so the
		// upstream sees only what the client actually sent.
		if err := replaceBody(stripCountTokensUnsupportedFields(body)); err != nil {
			return err
		}
	}
